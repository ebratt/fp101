...
-- recursive function that returns only if the entire target string
-- string is consumed
...
-- apply a parser p as many times as possible until it fails, with the
-- result values from each successful application of p being combined
-- as a list. many and many1 are defined using mutual recursion

-- many permits zero or more applications of p
many             :: Parser a -> Parser [a]
many p           = many1 p +++ return []

-- requires at least one successful application of p
...
-- Ignoring spacing around identifiers, natural numbers, and special 
-- symbols with token
